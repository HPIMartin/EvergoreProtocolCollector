...
|------|-------------|----------|----------------|
| **Planner** | You + me (the main-session model), in chat | No, it's a *hat* | Backlog item → an **approved, ordered commit/test list** + acceptance criteria (Gherkin if user-facing) |
| **Implementer** | `implementer` subagent (frontmatter `model: sonnet`) | Yes | Approved plan → executes red→green→refactor per step, **commits each locally** with the pre-approved message → summary |
| **Falsifier panel** | `falsifier-domain` + `falsifier-robustness` subagents (frontmatter `model: sonnet`) | Yes (both fresh) | The feature diff/commits → two adversarial verdicts (domain correctness · test honesty/robustness) + concrete counter-tests / weaknesses |
| **Reviewer / Gate** | `reviewer` subagent (frontmatter `model: opus`) | Yes (fresh) | The feature commits + the panel's reports → PASS/FAIL + findings + proposed `process-learnings.md` entry |

**Non-negotiable:** the falsifiers and the Reviewer are **fresh, independent** agents, never the Implementer
...
1. PLAN (you + me, chat)
   ...
   - (user-facing) write Gherkin scenarios, disabled until implemented
...
3. FALSIFY (spawn the panel: `falsifier-domain` + `falsifier-robustness`, both fresh)
   ...
   - each returns a verdict + proposed counter-tests (parallel only in separate worktrees, else sequential)
4. REVIEW GATE (spawn `reviewer`, fresh), once per FEATURE commit
   - process adherence (real red→green→refactor? whitespace separate? KB updated? commit-msg rules?)
   - code criteria (clean code, SOLID, architecture boundaries); integrates the panel's findings
...
commit**, not every micro-commit. The **falsifier panel** runs at the feature end. **Escalation
rule:** for features touching the project's high-risk areas, the planner spawns the gate agents with
a one-off `model` override on the strongest tier the plan offers (`fable` when available, otherwise
`opus`); the frontmatter defaults stay unchanged.

> **ADAPT:** name the high-risk areas that trigger gate escalation (typical candidates: money or valuation math, time/timezones, concurrency, data migration).
...
- **State the full worktree identity at every review-gateway hand-off:** the branch name **and** the
  absolute worktree path. The author opens that path directly in their editor to inspect the diff, so
  the hand-off must contain everything needed to do so; a hand-off missing either stalls the gateway.
- **Rebase autonomously, merge conditionally:** rebase a feature strand onto the current `main`
  without asking first. If the strand was already author-reviewed **and** the rebase ran clean (no
  conflicts, no judgment-call resolutions), fast-forward-merge it without a further review
  round-trip. Only stop for the author when the rebase hit a conflict: resolve it, verify the build,
  present exactly what was resolved plus the full branch-vs-main diff, and wait for the go. Conflict
  resolutions are the one place new, unreviewed content can appear, so human review stays exactly
  there. Pushing always stays with the author.
- **Delete merged branches immediately:** once a strand is fast-forward-merged into `main`, remove
  its worktree and delete its branch right away, without asking. Use the safe form `git branch -d`,
  which itself refuses if the branch is not fully merged. The commits live on in `main`, the branch
  label and worktree are pure redundancy, and the reflog still holds deleted tips for a while.

## How to invoke

The **orchestrator is the main session** (me). I drive the pipeline with the `Agent` tool,
`subagent_type` = `implementer` / `falsifier-domain` / `falsifier-robustness` / `reviewer`
(defined in `.claude/agents/`). The
...
- **Reviewer:** `PASS|FAIL`; findings `{category: process|cleancode|solid|architecture|tests|security,
  where, fix}`; a ready-to-paste `process-learnings.md` row if any process rule was missed.

## Environment gotchas (tell every agent)

- `git reset` (all forms) and `git branch -D` are permission-blocked (see §FAIL-loop for the
...
- **Never overlap a tree-mutating agent with another agent's build.** The falsifier rewrites source to
  ...
  in its own worktree (`Agent` `isolation: "worktree"`). The same holds inside the panel: the two
  falsifiers run sequentially, or each in its own worktree.
- Run **focused** tests during micro-steps ({{TEST_CMD}} scoped to a single test class), full
  {{BUILD_CMD}} before the gate.

> **ADAPT:** add machine-specific quirks agents must know (example: a host where Bash stdout is not surfaced, so agents redirect output to a file, `{{BUILD_CMD}} > out.txt 2>&1`, and `Read` it).

## Worked example: a small calculation feature (example)

1. **Plan:** test list for a value evaluator, e.g. ① happy-path value follows the documented formula
...
3. **Falsify (panel):** the domain lens recomputes ① and ② by hand; the robustness lens asks "does ③
   assert the log? does the zero-quantity edge work? is green real if I revert the production line?"
   → counter-tests.
4. **Review:** boundaries clean? KB ([domain-model.md](domain-model.md) / [testing.md](testing.md))
