...
|---|-----------|-------|-------------------|
| M1 | Data integrity | B15, B14, B11 | An aborted run under-counts permanently and the game serves only ~30 days of logs, so waiting loses data irreversibly (decision 2026-07-17) |
...
- [ ] The `Einzahlung`→`EINLAGERUNG` mapping and the value-neutral `+1` merge pinned by contract
      tests and documented (backlog B11).
