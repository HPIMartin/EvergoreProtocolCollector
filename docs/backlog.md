...
**Next action:** the **data-integrity milestone**: **B15** (meta sums recomputed from the stored
entries every run plus the ingest boundary-minute fix; design decision 2026-07-17) with **B14**
and the parser pins **B9**/**B11** (decision 2026-07-17: integrity before dashboard, an aborted
run under-counts permanently). Then **E5**'s remaining strands **`json-api`** → **`spa-views`** (decision 2026-07-04),
...
| **B9** | **Parser: pin `Einzahlung`→`EINLAGERUNG` with a contract test** (author decision 2026-07-17: `Einzahlung` occurs in real protocols; no distinct behavior warranted) and document the mapping in domain-model.md | An undocumented silent mapping reads like a bug and invites re-discovery | Contract test pins the mapping; domain-model.md documents it | S |
| **B11** | **Parser: pin the `+1` merge as intended** (author decision 2026-07-17: a `+1` item is value-equal to its base): a contract test for the merge consequence (`3 X +1` and `5 X` become one item, qty 8) plus a domain-model.md note | The merge consequence is undocumented and reads like a defect | Merge contract test; KB note | S |
...
| **B15** | **Meta sums: recompute from all stored entries every run; fix the ingest boundary-minute loss** (design decision 2026-07-17, replacing the earlier watermark rework): each evaluation recomputes the per-avatar sums from the stored entries and overwrites the meta values; `last_updated` becomes display-only, written after a successful run; the extractor ingests the boundary minute with occurrence-counting row deduplication instead of dropping everything at or before the stored max (two identical same-minute entries are legitimate in the game) | The cumulative sums are never recomputed today, so every over/under-count is permanent; ingest dedup never used the watermark, so recomputing removes the DST/skew/same-minute/partial-write defect class at the root and heals historical drift | Evaluation is idempotent (a second run yields identical sums) and self-healing after a mid-run failure; boundary-minute entries survive ingest; covered by deterministic tests | M |
