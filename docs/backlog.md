...
| **B12** | **Watermark timezone bug corrupts cumulative sums** (setup review 2026-07-03, confirmed): `EvergoreDataEvaluator` takes `LocalDateTime.now()` (system default zone, no injected `Clock`), the watermark serializes minute-precision (`dd.MM.yyyy HH:mm`), and both repos read it back as `APP_ZONE` (Berlin). In a UTC container (default browser is `docker`) a 2 h window is ingested twice into the never-recomputed meta sums. Fix: inject `Clock`, store an ISO instant (or better, derive the watermark from the max ingested entry timestamp); test the evaluate cycle under a non-Berlin default zone | Corrupts the headline numbers a little more on every run | Watermark is zone-safe; a test under a non-Berlin zone shows no double-count | S |
| **B13** | **`SeleniumPageSource` leaks the WebDriver on scrape failure** (setup review 2026-07-03): the driver is ended only on the happy path, via `close()` instead of `quit()`, in a swallowed catch outside try/finally; while scraping keeps failing, the 24 h job leaks one Firefox process per day. The empty catches also swallow all diagnosis | Resource leak plus silent failure in the most fragile adapter | `quit()` on all paths (try/finally); failures logged; repeated failure leaks no process | S |
