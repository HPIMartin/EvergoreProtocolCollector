...
React views against it. The **bug track**'s first three confirmed bugs (malformed-date ingest abort,
watermark timezone double-count, WebDriver leak) **landed 2026-07-09**, filing the follow-ups
**B15**/**B16**/**B17**; the remaining confirmed-bug work is the domain confirmations **B9**/**B11**.
Then the standing next-up set (all land *in Gradle*): **E1** (erzeugter Gildenmehrwert, the headline
metric, now easy to TDD on this harness), **H6** (failsafe → Gradle integration-test set); **H9** (jump
to Micronaut 5) only *after* 1:1 is re-proven. Plan via the agent pipeline (planner → implementer →
...
- **Definition of Done:** compiles · unit-tested · KB doc updated · committed in a focused commit ·
  no new hard-coded secrets · local build green (no CI; handbook §8).
...
| **B14** | **Item catalog typo makes an item worthless**: `"Erde-EIbenlanze"` in `EvergoreItem`; the real spelling is `Erde-Eibenlanze` (author-confirmed 2026-07-03). Exact-name matching sends the real item to `UNDEFINED`, value 0, info-log only. Fix the constant, and raise unknown-item handling from info to **warn plus a counter in the health detail** (silently valuing unknown items at 0 is a data-loss risk) | Valuation correctness; unknown items must be loud | Constant fixed with a test; unknown items warn and are countable via `/health` | S |
| **B15** | **Watermark is derived from `now()`, not the max ingested entry timestamp** (verification findings 2026-07-08, beyond the watermark zone fix that just landed): the watermark still uses `LocalDateTime.now(clock)` at minute precision, leaving four residual defects the zone fix does not cover — (a) during the Berlin DST fall-back overlap the read side resolves to the earlier offset, re-ingesting up to 60 min once a year; (b) `getAllFor(avatar, after)` has no upper bound, so an entry stamped after `now` (clock skew) is summed this run and again next; (c) `advanceWatermarkAndReturnPrevious()` writes the new watermark *before* `updateAvatarInformation`, so a mid-run failure permanently under-counts (non-transactional, never recomputed); (d) minute truncation drops a same-minute-but-not-yet-visible entry forever. Fix: derive the watermark from the max ingested entry timestamp (subsumes a+b+d) and advance it only after a successful update (fixes c) | The cumulative meta sums are never recomputed, so every residual over/under-count is permanent | Watermark sourced from the max ingested entry; advanced only after a successful update; DST/skew/same-minute covered by tests | M |
...
| ID | Item | Why | Acceptance | Effort |
|----|------|-----|------------|--------|
| **F1** | **Remove the bundled webdriver binaries and the local-browser machinery** (author decision 2026-07-03): 87 MB of Jan-2023 gecko/chrome/edge binaries in `src/main/resources` ship in every jar and image and are never used (default browser is `docker`; the 2023 builds cannot drive current browsers anyway). The `CHROME`/`EDGE`/`FIREFOX` local paths and the loader/driver machinery go with them; **H2**'s remote driver becomes the only path. Afterwards the author rewrites history himself (`git filter-repo`; the pack drops from 66.7 MiB; breaks clones and hashes, accepted). Scrape resilience (retries, selector config, fail loudly) stays in scope | 87 dead MB in every artifact and permanently in the pack; obsolete driver tech | No binaries under `src/main/resources`; local-browser code gone; scraping works via H2's service; history rewritten by the author | M |
