...
| **D7** | **Make the application use-cases framework-free** (conformance-audit finding): move `EvergoreDataExtractor`/`EvergoreDataEvaluator`/`LastRunStatus` into an `application` layer, drop their `@Singleton` (wire them in `ApplicationFactory` like the repositories), and **extend the ArchUnit guard** to forbid `io.micronaut`/`jakarta` in the use-case package + enforce inner→outer package direction | The "application layer is framework-free" goal isn't met yet, and the guard covers only `domain`/`businessLogic` | Use-cases carry no framework annotation; ArchUnit fails on a framework import there; behaviour unchanged | M (slice of **D3**) |
| **D8** | **Value objects → records** (conformance-audit finding): convert `BankEntry`/`StorageEntry`/`MetaInformation` to `record`s and remove the `stupidMerge()` placeholder (use generated `equals`/`hashCode`) | Documented "prefer immutable records"; removes a joke-named method the handbook says to clean up | All three are records; **golden-master-guard the dedup** (`equals`/`hashCode` drives entry de-duplication in `EvergoreDataExtractor`) — 1:1 unchanged | S–M |
