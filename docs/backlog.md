...
| **C9** | **Keep the user-supplied value out of the HTTP reason phrase** (reviewer finding 2026-08-04): `ApplicationExceptionHandler.onNoElementFound` puts the requested path variable into the reason phrase, so an unknown avatar carrying CRLF makes Netty refuse the response with a 500 plus an `error`-level stack trace. No response splitting is possible (Netty rejects it), but it is a caller-triggerable server error and log-noise amplifier, now reachable on the JSON route as well as the legacy HTML one | An input-shaped 500 with a stack trace is neither a correct status nor an expected-client-error log line | The status carries no user-supplied text; an unknown avatar with control characters answers 404 and logs one `info` line | S |
| **C10** | **A malformed request target is answered before the rate limiter and the request log see it** (falsifier finding 2026-08-05, verified in the `json-api` gate): Micronaut answers `/overview%zz` (any invalid percent-escape) with **400** ahead of the filter chain, so `BrowserLoggingFilter` neither counts nor logs it. A caller can therefore send unlimited malformed requests without appearing in the request log or the per-IP counter. Not fixable in our filters, which never run: needs a Netty/Micronaut-level seam (or an accepted-and-documented limitation). Pinned by `RateLimitFilterTest.doesNotCountAMalformedRequestTargetBecauseTheFrameworkAnswersItFirst` and by a five-target `@ParameterizedTest` in `TokenScopeTest`; both **characterize Micronaut, not our filters**, and go red if a framework upgrade changes the behaviour. Verified 2026-08-05 by log evidence: a malformed target produces no `Client IP` line at all, while the well-formed control `/overview%2e%2e` answers 401 and logs | An unlogged, uncounted request path defeats both the throttle and the audit trail | A malformed target is either counted and logged like any other request, or the gap is an explicit decision with a documented rationale | S |
...
| **G11** | **Improve the AI/agent working environment in the devcontainer** (low prio): pre-allow common read-only commands in the committed `settings.json` (portable forms only), and add conventions/hooks that cut token use (scratch-file hygiene, scoped reads over blind re-scans) | Recurring friction: repeated permission prompts, the "always allow" flow re-polluting `settings.json`, broad re-reads burning tokens | P3 | Author request 2026-06-16. Sits with **G7**/**G10** (the hooks/showcase items). The tool pre-installs are done (2026-08-01: `bc`, `git-filter-repo`, `sqlite3`, `firefox-esr` in the image; `gh` and `docker` as features); open are the `settings.json` pre-allows and the token-cutting hooks. **Author decision 2026-08-05, next strand after `json-api`:** add a **gitignored `src/probe/java` source set** carrying the test classpath plus its own `./gradlew probe` task that is *not* wired into `check`/`build`, and point the falsifier agents at it for throwaway probes. Today a probe lands in `src/test/java`, so it can turn `build` red and `git add -A` can commit it, which puts the author's `rm` on the critical path of every gate round (it happened twice during the `json-api` gate). With the new location a leftover probe is invisible to both git and the build. |
