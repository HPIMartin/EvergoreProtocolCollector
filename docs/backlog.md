...
| **C9** | **Log expected client errors without a stack trace** (finding 2026-07-31, running the distribution locally): `ApplicationExceptionHandler.handle` logs `logger.error(reason, exception)` *before* visiting, so the stack-trace clearing in `onAccessNotAllowed`/`onTooManyRequests` only shapes the response, never the log. Every unauthorized or rate-limited request writes a full Netty stack trace at ERROR, so probing the token inflates the log and buries real errors. Fix: decide the severity from the mapped response (client status → one-line `info`, server status → `error` with the trace), which also removes the now-pointless `setStackTrace(new StackTraceElement[0])` calls | An expected 401 is not an application error; noise at ERROR defeats the log as a monitoring surface | An unauthorized request logs one line without a stack trace; a 500 still logs the trace; covered by tests | S |
