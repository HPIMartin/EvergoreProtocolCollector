...
| **D4** | Unify the two `TransferType→String` visitors; replace `ApplicationExceptionHandler` `instanceof` chain with a visitor | Remove duplication & the pattern the project is eliminating | One mapping source; handler has no `instanceof` | S |
...
| **G15** | **Conformance audit — are the defined rules actually applied in the code?** A one-off checkup that the codebase matches the documented conventions: TDD + test structure ([testing.md](knowledge-base/testing.md)), hexagonal boundaries beyond the existing ArchUnit guard ([architecture.md](knowledge-base/architecture.md)), immutable records + Logger-last constructor param, comment minimalism, warnings-as-errors, English-only identifiers, AssertJ assertion idiom, and no hard-coded secrets/host paths. | The rules are written down but only **partly** mechanically enforced (pre-commit hook + ArchUnit cover a subset); the rest (test layout, naming, record immutability, Logger-last, comment minimalism, SOLID adherence) drift invisibly until audited — and "the code follows its own rules" is the showcase's core claim. | P2 | Author request 2026-06-26. **Execute as parallel subagents grouped by rule-area** (one group ≈ a few related rules) — most groups are mechanical pattern checks that **don't need Opus/high effort**; reserve the strong model for the architecture/SOLID judgment calls. Output = a per-group findings report (conforming vs `file:line` violations); fix trivial violations inline, file the rest as backlog items. Natural fit for the multi-agent playbook ([multi-agent-playbook.md](knowledge-base/multi-agent-playbook.md)). |
| **G16** | **De-Claude-ify the docs + thin CLAUDE.md to a wrapper.** From the 2026-06-26 tool-agnosticism audit: (1) replace CLAUDE.md's duplicated commit-protocol / TDD-BDD / DoD / golden-rule bodies with pointers into the KB (the duplication already drifts from [engineering-handbook.md](knowledge-base/engineering-handbook.md) §7 — it omits §7's LF/`.gitattributes` + hooks-active points); keep only genuine Claude-Code mechanics. (2) Neutral wording: "the reviewer flags" → "a reviewer (human or agent)", "all agents" → "all contributors"; rename [working-with-claude.md](knowledge-base/working-with-claude.md) → `working-with-ai-agents.md` + fix pointers. (3) Lift tool-neutral facts out of Claude-only docs: the gviz CSV endpoint → [google-sheet.md](knowledge-base/google-sheet.md), the FAIL-loop governance → the handbook. (4) Add a neutral `agent-entry-template.md` to the KB + a "step 0" rule: any tool without a native entry file builds its thin wrapper from the template, records the template-version, and quick-checks at session start that its shared section still matches (re-sync on a version bump). | KB already holds the canonical rules, but CLAUDE.md duplicates them (drift started) and a few tool-neutral facts sit in Claude-only docs — a second tool (e.g. a local LLM) or a human reading only the KB gets an incomplete picture. A single-source KB + a templated per-tool wrapper future-proofs the multi-tool showcase. | P2 | Author request 2026-06-26 (doc-agnosticism audit). Sibling to **G14** (de-shortcode) — do together/back-to-back. No knowledge MOVES out of the KB (it is already there); the work is de-duplication + neutral phrasing + the template/step-0 mechanism. Precedent is set by the worktree-workflow codification landing first (branching canonical in §7, CLAUDE.md only points). |
