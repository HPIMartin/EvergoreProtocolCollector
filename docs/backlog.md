...
**Next action:** the **data-integrity milestone**: **B15** (the ingest boundary-minute fix; the
meta-sums recompute half just landed, design decision 2026-07-17). Then **E5**'s remaining strands
**`json-api`** → **`spa-views`** (decision 2026-07-04), then **B16**/**B17**, **E1**/**E2**,
**H2**+**H6**; **H9** only *after* 1:1 is re-proven. Milestone cuts + acceptance:
[roadmap.md](roadmap.md); risk register: [risks.md](risks.md). Plan via the agent pipeline
(planner → implementer → falsifier panel → reviewer). *(A4/CI stays deprioritized: local-only
...
| **B15** | **Ingest boundary-minute fix** (design decision 2026-07-17; the meta-sums recompute half of this item has landed): the extractor drops an entry sharing the stored max timestamp if it becomes visible only after that scrape (strict is-after cutoff); fix by ingesting the boundary minute with occurrence-counting row deduplication (two identical same-minute entries are legitimate in the game) instead of dropping everything at or before the stored max | Ingest dedup never used the watermark, so a boundary-minute entry is permanently lost if it becomes visible only on a later scrape | Boundary-minute entries survive ingest; covered by deterministic tests | M |
