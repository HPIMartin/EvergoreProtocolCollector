...
How we develop this project with a small team of AI agents: **you + I plan; cheaper agents
...
| **Falsifier panel** | `falsifier-domain` + `falsifier-robustness` subagents (frontmatter `model: sonnet`) | Yes (both fresh) | The feature diff/commits → two adversarial verdicts (domain math · test honesty/robustness) + concrete counter-tests / weaknesses |
...
3. FALSIFY (spawn the panel: `falsifier-domain` + `falsifier-robustness`, both fresh)
   - domain lens: value math, watermark/aggregation, parser fidelity; robustness lens: fake-green
...
4. REVIEW GATE (spawn `reviewer`, fresh), once per FEATURE commit
   ...
   - code criteria (clean code, SOLID, hexagonal); integrates the panel's findings
...
**Cadence (cost vs rigor, decided 2026-06-13; panel + escalation decided 2026-07-03):** micro-steps
run lightweight; the **reviewer gates at the feature commit**, not every micro-commit. The
**falsifier panel** runs at the feature end. **Escalation rule:** for features touching valuation
math, time/timezones, concurrency or data migration, the planner spawns the gate agents with a
one-off `model` override on the strongest tier the plan offers (`fable` when available, otherwise
...
3. **Falsify (panel):** the domain lens recomputes ① and ② by hand; the robustness lens asks "does ③ assert the log? does quality=0 edge work? is green real if I revert the production line?" → counter-tests.
4. **Review:** boundaries clean? KB `domain-model.md`/`testing.md` updated? messages one-line verb-first? → PASS.
