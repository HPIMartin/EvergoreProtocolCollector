...
| `StoredAvatarReconciliationTest` | The repeatable check behind the avatar/type-word fix, the only parser change that can alter the parse output of an already-stored entry: no stored avatar is blank or contains a protocol type word, so no stored row was minted by the greedy avatar group latching onto one (such a row re-parses differently and the ingest-window dedup re-ingests it as a duplicate). Runs twice, over the committed fixture and over the gitignored production snapshot at `temp.sqlite`, the latter `assumeTrue`-skipped when no snapshot is there; both copy the database into `build/tmp/reconciliation` first, because the repositories create tables on construction (same rule as `ProductionSnapshotRecomputeCheck`). Each database case asserts the avatar list is **non-empty** before judging it, so a wrong or empty database fails instead of passing on nothing, and the detector is pinned separately against literal suspect and ordinary names. **It errs in both directions on purpose:** it reads stored avatars, so it cannot see a headline whose misread type word left an ordinary-looking avatar behind, and it flags by substring, so a member legitimately named after a type word turns it red with nothing wrong. A red means "inspect these names", not "defect". | pure unit + fixture and snapshot databases |
