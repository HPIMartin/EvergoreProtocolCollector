...
| `BankDatabaseRepositoryTest` | Repository is usable without separate init (file DB); `getAllSince(timestamp)` includes the row exactly at the boundary, excludes older rows, includes newer rows; `countFor` counts only the given avatar's rows and is zero for an unknown one; `latestTimestampPerAvatar` names the newest row per avatar and nobody at all for an empty ledger (in-memory SQLite). | adapter integration |
