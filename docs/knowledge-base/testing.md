...
| `AvatarContributionsTest` | Pure unit tests over the assembler: all four stored sums of a known avatar are read, an avatar without any stored sum counts as zero instead of dropping out of the list, and the German collation order of `KnownAvatars` is kept. Hand-written fakes: `BankRepositoryStub`, `StorageRepositoryStub`, `FakeMetaInformationRepository`. | pure unit |
