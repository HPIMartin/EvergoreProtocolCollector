...
| `StoredAvatarReconciliationTest` | The check behind the avatar/type-word fix, the only parser change that can alter the parse output of an already-stored entry: no stored avatar is blank or contains a protocol type word, so no stored row was minted by the greedy avatar group latching onto one (such a row re-parses differently and the ingest-window dedup re-ingests it as a duplicate). Reads the committed fixture only, and copies it into `build/tmp/reconciliation` first, because the repositories create tables on construction and the fixture is a tracked file. It asserts the avatar list is **non-empty** before judging it, and the detector is pinned separately against literal suspect and ordinary names. **It errs in both directions on purpose:** it reads stored avatars, so it cannot see a headline whose misread type word left an ordinary-looking avatar behind, and it flags by substring, while an avatar name may legitimately contain a type word. A red means "inspect these names", not "defect". | pure unit + fixture database |
