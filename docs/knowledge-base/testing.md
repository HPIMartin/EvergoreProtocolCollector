...
| `AvatarSummariesControllerTest` | Pure unit tests over the stubbed repositories: the overview lists an avatar that only ever moved items, `totalCount` counts every known avatar while a page shows only part of them, an avatar without any stored sum carries zeros, a seeded avatar's four ledger sums plus the derived net are served as whole gold, and the envelope's `totals` cover **every** known avatar rather than only the served page. Uses `KnownAvatars` over the two repository stubs plus `FakeMetaInformationRepository`. | pure unit |
