...
| `EntityParserContractTest` | The parser contract, pinned line by line: headline → entry, `Einzahlung`→`EINLAGERUNG`, withdrawal entries, quality defaulting to 100 when absent, the value-neutral `+1` modifier (ignored, and merged with its unmodified counterpart), quantity merging for same name+quality, separation when quality differs, the `Impressum` terminator, one entry per headline, empty protocol → no entries, and the malformed/out-of-range-date paths (skipped without aborting the ingest, neighbouring entries still parsed, the malformed headline's items **not** folded into the preceding entry, a warning when a typed headline is dropped), and the unparseable-item-line path (an amount or quality that is no parseable number skips **that line only**, with a warning; the entry's other items and the neighbouring entries survive), and the no-parseable-items path (an entry whose item lines all fail the item regex warns). | pure unit |
