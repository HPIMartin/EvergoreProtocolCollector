...
| `AvatarContributionsTest` | Pure unit tests over the assembler: all four stored sums of a known avatar are read, an avatar without any stored sum counts as zero instead of dropping out of the list, the German collation order of `KnownAvatars` is kept, and each ledger's newest entry becomes that avatar's last activity while a ledger he never used stays unanswered. Hand-written fakes: `BankRepositoryStub`, `StorageRepositoryStub`, `FakeMetaInformationRepository`. | pure unit |
